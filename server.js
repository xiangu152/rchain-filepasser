...
app.post('/getInfo', (req, res) => {
  let rho_code = `new getfile, setfile, retfile in {
    @{"global_factory"}!(*getfile, *setfile)
    |
    getfile!(*retfile)
}`;
...
app.post('/setInfo', (req, res) => {
  let rho_code = `new getfile, setfile in {
    @{"global_factory"}!(*getfile, *setfile)
    |
   setfile!({"file":"${req.body.file}","pwd":"${req.body.pwd}","filename":"${req.body.filename}"})
}`;
