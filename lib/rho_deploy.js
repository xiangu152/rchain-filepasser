...
const func_deploy = async (rho_code_, order_) => {
  ...
  if(order_ >= 0){
    ...
    let data;
    console.log( JSON.stringify(data_json.expr.ExprMap.data.file.ExprString.data)+ '\n');
    if(data_json != null){
 //     data = rchainToolkit.utils.rhoValToJs(
  //      JSON.stringify(data_json.expr.ExprMap)
   //   );
   	data=JSON.stringify({
   	"file":data_json.expr.ExprMap.data.file.ExprString.data,
   	"pwd":data_json.expr.ExprMap.data.pwd.ExprString.data,
   	"filename":data_json.expr.ExprMap.data.filename.ExprString.data
   	})
    } else {
      data = null;
    }
